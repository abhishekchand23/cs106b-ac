...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    Map<string, double> kGramsOut;

    if (str.length() < kGramLength & kGramLength > 0) {
        return kGramsOut; //default initialization works here

    } else if (kGramLength <= 0) {
        error("kGramLength is not valid");

    } else {
        string tempStr = str;
        while (tempStr.length() >= kGramLength) {
            string tempGram = tempStr.substr(0,kGramLength);
            kGramsOut[tempGram] += 1;
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    if (input.isEmpty()) error("Map is empty");
    else {
        Vector<double> freq = input.values();
        double sq_sum = 0.0;
        for (double elem: freq) {
            sq_sum += pow(elem, 2);
        }
        if (sq_sum == 0) error("Map has 0 freq");

        double unit_vec = sqrt(sq_sum);
        Map<string, double> tempInput = input;
        for (string key: tempInput) {
            tempInput[key] /= unit_vec;
        }
...
//    use priority queue to sort the map

    if (numToKeep < 0) error("The nunber to keep is less than zero");
    if (numToKeep == 0) return {};
    if (numToKeep >= source.size()) return source;
    else {
        Map<string, double> tempGram;
        int numToRemove = source.size() - numToKeep;

        //make priority queue using the source map
        PriorityQueue<string> pq;
        for (string key: source) {
            pq.enqueue(key, source[key]);
        }

        //remove low freq kgrams from pq
        while (numToRemove > 0 ){
            pq.dequeue() ;
            numToRemove-- ;
        }

        //add elements to tempGram from leftover pq
        while (!pq.isEmpty()) {
            tempGram[pq.peek()] = pq.peekPriority();
            pq.dequeue();
        }
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    double cosine = 0.0;
    for (string key: lhs) {
        if (rhs.containsKey(key)) cosine += lhs[key] * rhs[key] ;
    }
    return cosine;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    Map<string, double> cosineLang;
    for (Corpus elem: corpora) {
        cosineLang[elem.name] = cosineSimilarityOf(textProfile, elem.profile);
    }
    return topKGramsIn(cosineLang, 1).firstKey();
}
