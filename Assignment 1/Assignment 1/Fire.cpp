...
void updateFire(Grid<int>& fire) {
     ...
     */
    int n_rows = fire.numRows();
    int n_cols = fire.numCols();


    for (int i = 1; i < n_rows; i++) {
        for (int j = 0; j < n_cols; j++) {

            int temperature = fire.get(i, j);
            int prev_row = i - 1;
            int col_selected;
            float prob_dec = (float) 2 / 3 ;

            if (j==0 && n_cols < 2){
                col_selected = j;
            }
            else if (j == 0 && n_cols > 1) {
                col_selected = randomInteger(j, j + 1);
            } else if (j == (n_cols - 1) ) {
                col_selected = randomInteger(j, j - 1);
            } else {
                col_selected = randomInteger(j - 1, j + 1);
            }

            if (temperature == 0){
                fire.set(prev_row, col_selected, 0);
            } else {
                if (randomChance(prob_dec)) {
                    fire.set(prev_row, col_selected, temperature - 1);
                } else {
                    fire.set(prev_row, col_selected, temperature);
                }
            }
        }
